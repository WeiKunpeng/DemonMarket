...
import com.tining.demonmarket.data.Trade;
...
public class MarketGetInput {
    ...
    public static void message(MarketTrade.type type, Player player) {
        switch (type) {
            case BUY: {
                player.sendMessage(ChatColor.AQUA + "[DemonMarket]请按下T并输入你想购买的数量");
                break;
            }
            case SELL: {
                player.sendMessage(ChatColor.AQUA + "[DemonMarket]请按下T并输入你想出售的数量，输入all可出售身上所有物品");
                break;
            }
        }
    }
